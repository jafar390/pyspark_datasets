...
#stages21 += [vector_assembler]
#stages21 += [minmax]
stages291 += [rf]

from pyspark.ml import Pipeline

pipeline291 = Pipeline().setStages(stages291)
rf7_model291 = pipeline291.fit(pca_train)

rf7_pp_df291 = rf7_model291.transform(pca_test)
rf7_predicited291 = rf7_pp_df291.select("prediction","label","rawPrediction","probability")
tp = float(rf7_predicited291.filter("prediction == 1.0 AND label == 1").count())
fp = float(rf7_predicited291.filter("prediction == 1.0 AND label == 0").count())
tn = float(rf7_predicited291.filter("prediction == 0.0 AND label == 0").count())
fn = float(rf7_predicited291.filter("prediction == 0.0 AND label == 1").count())

acc = float((tp+tn)/rf7_predicited291.count())
pr = tp / (tp + fp)

re = tp / (tp + fn)

metrics291 = spark.createDataFrame([("TP",tp),("FP",fp),("TN",tn),("FN",fn),("accuracy",acc),("precision",pr),("Recall",re),("F1",2*pr*re/(re+pr))],["metric","value"])
metrics291.show()
