...
logger.error("######### Gradient Boosting algorithm")
...
gbn7_model20912 = pipeline20912.fit(pca_train)

gbn7_pp_df20912 = gbn7_model20912.transform(pca_test)
gbn7_predicited20912 = gbn7_pp_df20912.select("prediction","label","rawPrediction")
tp = float(gbn7_predicited20912.filter("prediction == 1.0 AND label == 1").count())
fp = float(gbn7_predicited20912.filter("prediction == 1.0 AND label == 0").count())
tn = float(gbn7_predicited20912.filter("prediction == 0.0 AND label == 0").count())
